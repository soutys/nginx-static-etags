/*
 *  Copyright (c) 2013 soutys ( https://github.com/soutys/ ).
 *  All rights reserved.
 *  All original code was written by Mike West ( http://mikewest.org/ )
 *  Extended by Adrian Jung ( http://me2day.net/kkung )
 *
...
#define ETAG_HEADER_FIELD_NAME  "Etag"
#define ETAG_HASH_METHOD_MD5    "md5"
#define ETAG_HASH_METHOD_SHA1   "sha1"
...
static char *ngx_http_static_etags_hash_method(ngx_conf_t *cf,
...
static void * ngx_http_static_etags_create_loc_conf(ngx_conf_t *cf) {
	...
	if (conf == NULL) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
			"[etag] failed memory allocation (options)");
		return NULL;
	}
...
static char * ngx_http_static_etags_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child) {
	...
	ngx_conf_merge_str_value(conf->hash_method, prev->hash_method, ETAG_HASH_METHOD_MD5);
...
static char *ngx_http_static_etags_hash_method(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
	...
	if (l_conf->hash_method.len > 0) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "[etag] duplicated definition");
		return NGX_CONF_ERROR;
	}
	
	ngx_str_t *value = cf->args->elts;
	if ( ngx_strcmp(value[1].data, ETAG_HASH_METHOD_MD5) == 0 ) {
		l_conf->hash_method = value[1];
	} else if ( ngx_strcmp(value[1].data, ETAG_HASH_METHOD_SHA1) == 0 ) {
		l_conf->hash_method = value[1];
	} else {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "[etag] invalid value - only "
			ETAG_HASH_METHOD_MD5 " or " ETAG_HASH_METHOD_SHA1 " allowed");
		return NGX_CONF_ERROR;
	}
...
static ngx_int_t ngx_http_static_etags_header_filter(ngx_http_request_t *r) {
	...
	size_t                            root;
	ngx_str_t                         path, str_buffer, etag_token;
	...
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "[etag] enabled? \"%d\"", loc_conf->enable);
	...
	if (last == NULL) {
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, log, 0,
			"[etag] unable to map uri to path");
	...
	int status = stat( (char *) path.data, &stat_result );
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, log, 0,
		"[etag] stat returned: \"%d\" @ \"%s\"", status, path.data);
	// Did the `stat` succeed? if there was error - just skip processing
	if (status != 0) {
		return ngx_http_next_header_filter(r);
	}
	...
	if (clcf == NULL) {
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, log, 0,
			"[etag] failed memory allocation (core conf)");
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}
	...
	if (ngx_open_cached_file(clcf->open_file_cache, &path, &of, r->pool) == NGX_OK) {
		...
		// 3 (three) variables + 2 (two) underscore chars + 2 (two) quote chars + 1 (one) '\0' char
		str_buffer.data = ngx_palloc(r->pool,
			r->uri.len + sizeof(of.size) + sizeof(of.mtime) + 5
		);
		if (str_buffer.data == NULL) {
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, log, 0,
				"[etag] failed memory allocation (etag buf)");
			return NGX_ERROR;
		}

		// quoted string in case of disabled `enable_hash`
		str_buffer.len = ngx_sprintf(str_buffer.data, "\"%V_%T_%z\"",
			&r->uri, of.mtime, of.size) - str_buffer.data;
		str_buffer.data[str_buffer.len] = '\0';
		...
		ngx_table_elt_t *new_etag;
		if (r->headers_out.etag == NULL) {
			// create new ETag
			new_etag = ngx_list_push(&r->headers_out.headers);
			if (new_etag == NULL) {
				ngx_log_debug(NGX_LOG_DEBUG_HTTP, log, 0,
					"[etag] failed new header allocation");
				return NGX_ERROR;
			}
			new_etag->hash = 1;
			ngx_str_set(&new_etag->key, ETAG_HEADER_FIELD_NAME);
		} else {
			// use existing ETag, clear header value
			new_etag = r->headers_out.etag;
			ngx_str_null(&new_etag->value);
		}
		...
		if (loc_conf->enable_hash) {
			...
			if ( ngx_strcmp(loc_conf->hash_method.data, ETAG_HASH_METHOD_MD5) == 0) {
			...
			} else if ( ngx_strcmp(loc_conf->hash_method.data, ETAG_HASH_METHOD_SHA1) == 0 ) {
				...
				if (hash == NULL) {
					ngx_log_debug(NGX_LOG_DEBUG_HTTP, log, 0,
						"[etag] failed memory allocation (sha1)");
					return NGX_ERROR;
				}
			...
			// hash + double quotes
			etag_token.len = hex_digest_length + 2;
			etag_token.data = ngx_palloc(r->pool, etag_token.len + 1);
			if (etag_token.data == NULL) {
				ngx_log_debug(NGX_LOG_DEBUG_HTTP, log, 0,
			...
			for (i = 0; i < digest_length; i++) {
				etag_token.data[i * 2 + 1] = hex[hash[i] >> 4];
				etag_token.data[i * 2 + 2] = hex[hash[i] & 0x0f];
			}
			etag_token.data[etag_token.len - 1] = '"';
			etag_token.data[etag_token.len] = '\0';

			ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "[etag] hash result %V",
				&etag_token);

			new_etag->value.len = etag_token.len;
			new_etag->value.data = etag_token.data;
		} else {
			new_etag->value.len = str_buffer.len;
			new_etag->value.data = str_buffer.data;
		} // if (loc_conf->enable_hash) ...

		ngx_uint_t found = 0;
		ngx_list_part_t *part;
		ngx_table_elt_t *header, *if_none_match;
		...
		if (found) {
			ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
				...
				&if_none_match->value, &new_etag->value);
			// compare without quotes...
			if ( ngx_strncmp(new_etag->value.data + 1,
				if_none_match->value.data, new_etag->value.len - 2) == 0 ) {
